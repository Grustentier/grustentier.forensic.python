...
'''
Extended enhance.py version with additional parameters and use of imageio
'''
...
add_arg('--inputdir', default='', type=str, help='The input directory with separated frames/images')
add_arg('--exportdir', default='', type=str, help='The export directory path') 
...
if __name__ == "__main__":
    if args.train:
        ...
        assert args.inputdir and len(args.inputdir) > 0 and os.path.isdir(args.inputdir), "Please check your input directory"
        assert args.exportdir and len(args.exportdir) > 0, "Please check your export directory"
        createExportDirs(args.exportdir)
        
        enhancer = NeuralEnhancer(loader=False)
        for root, _, files in os.walk(args.inputdir): 
            for file in files: 
                filenPath = root + "/" + file           
                # img = scipy.ndimage.imread(filenPath, mode='RGB')
                img = imageio.imread(filenPath)                
                out = enhancer.process(img)
                if str(args.exportdir).endswith("/"):
                    out.save(args.exportdir + file)
                else:
                    out.save(args.exportdir + "/" + file)
                print(flush=True)
            print(ansi.ENDC)
